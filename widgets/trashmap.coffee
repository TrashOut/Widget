...
(($, window, document) ->
  ...
  TrashMap.prototype =
      ...
      map:
        ...
        options:  # classic google mas options
          ...
          styles: [{  # special style - disable points and other annoying stuff
            ...
            stylers: [{ visibility: 'off' }] # the style rules to apply to the selected map features and elements. The rules are applied in the order that you specify in this array
          }]
        cluster:
    ...
    initMap: ->
      # init jquery object
      @map.object = $ @config.map.id

      # load map config from URL hash params
      @loadMap()

      # create gmap3 plugin object
      @map.object.gmap3(@config.map.options).on(
        tilesloaded: ->
    ...
    loadMap: () ->
      ...
      # zoom
      if data.length >= 3 and typeof data[2] != 'undefined'
        self.config.map.options.zoom = parseInt data[2]

      # gps position
      if data.length >= 2 and typeof data[1] != 'undefined' and typeof data[0] != 'undefined'
        self.config.map.options.center = [data[0], data[1]]
    ...
    # @return {Boolean} Returns true on success
    addCluster: (data, init = false) ->
      markers = []

      for i of data
        obj = data[i]
        if init or !(obj.id of self.data.trashlist)
          marker =  new (google.maps.Marker)(
          ...
          marker.addListener 'click', ->
            self.showTrashDetail this.data.id

          markers.push marker
          self.data.trashlist[obj.id] = obj

      styles = [
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '0.png' },
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '1.png' },
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '2.png' },
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '3.png' },
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '4.png' },
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '5.png' },
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '6.png' },
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '7.png' },
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '8.png' },
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '9.png' },
        { width: 60, height: 60, url: self.config.map.cluster.imagePath + '10.png' }
      ];
    ...
    getTrashList: (geocells) ->
      # test attributes
      if geocells == null or geocells.length == 0
        return false

      $.each geocells, (key, obj) ->
        self.data.trashlistGeocells[obj] = {}
      ...
      $.ajax(
        ...
        $.each data, (key, obj) ->
          self.data.trashlistGeocells[obj.geocell] = {}
        ...
        $.each geocells, (key, obj) ->
          delete self.data.trashlistGeocells[obj]
    ...
    # @return null
    processTrashListResponse: (response) ->
    ...
    drawTrashList: () ->
      ...
      if self.data.trashlist.length > 0 and self.map.cluster == null
        self.clearMap()
      ...
      data = self.getTrashList(geocells)
    ...
    # @return {Boolean} Returns true on success
    getZoomPoints: (zoomLevel, geocells) ->
      # test attributes
      if zoomLevel == null or geocells == null or geocells.length == 0
        return false

      if !self.data.zoompoints[zoomLevel] or typeof(self.data.zoompoints[zoomLevel]) == undefined
        self.data.zoompoints[zoomLevel] = []

      $.each geocells, (key, obj) ->
        self.data.zoompoints[zoomLevel][obj] = {}
      ...
      $.ajax(
        ...
        $.each data, (key, obj) ->
          self.data.zoompoints[zoomLevel][obj.geocell] = obj
        ...
        # when ajax call fails, delete all pre-cached data
        $.each geocells, (key, obj) ->
          delete self.data.zoompoints[zoomLevel][obj]
    ...
    # @param {object} API response
    # @return null
    processZoomPointsResponse: (response) ->
    ...
    updateMap: (clear = true) ->
      if !self.map.initialize
        return false

      map = @map.object.gmap3().get(0) # get google maps object

      if map.getZoom() <= @config.map.zoomDelimiter
    ...
    # @return {Boolean} Returns true on success
    clearMap: (type = 'all') ->
      if type == 'all' or type == 'trash'
        if self.map.cluster
          self.map.cluster.clearMarkers()
          self.map.cluster = null

      if type == 'all' or type == 'zoompoint'
    ...
    getMarkerIcon: (status, updateNeeded) ->
      imgPath = $.fn.config('system.paths.images') + '/trashmap/'
      if status == 'cleaned'
        imgPath + 'map_cleaned.png'
      else if updateNeeded
        imgPath + 'map_unknown.png'
      else
        imgPath + 'map_reported.png'
    ...
    filterGeocellsZoomPoints: (zoom, geocells) ->
      result = []
      $.each geocells, (key, obj) ->
    ...
    # @param {array} geocells - list of geocells for filtering
    # @return {array} of new geocells (which are not in local cache)
    filterGeocellsTrashList: (geocells) ->
      result = []
      $.each geocells, (key, obj) ->
    ...
    getGeocells: () ->
      ...
      geocell1 = geocells1[@getGeocellResolution(zoom)]
      ...
      geocell2 = geocells2[@getGeocellResolution(zoom)]
    ...
    renderFilter: () ->
      ...
      $headerWrapper = $('<h2/>', {'html': '<span id="close-filter"></span>' + i18n('global.filter') + '<span id="update-filter"></span>'})

      # render status
      $statusWrapper = $('<div/>', {'class': 'filter-wrapper'})
      $statusWrapper.append $('<div/>', {'class': 'title', 'html': i18n('trash.status')})
      $.each structure.status, (key, object) ->
        $statusWrapper.append $('<input/>', {'type': 'checkbox', 'id': key, 'name': 'filter-status[]', 'value' : key})
        $statusWrapper.append $('<label/>', {'for': key, 'html': i18n(object.name)})

      # render lastUpdate
      $lastWrapper = $('<div/>', {'class': 'filter-wrapper'})
      $lastWrapper.append $('<div/>', {'class': 'title', 'html': i18n('trash.filter.dateOfLastUpdate')})
      $lastWrapper.append $('<input/>', {'type': 'range', 'id': 'filter-last', 'name': 'filter-last', 'min': 0, 'max': 100, 'step': 25, 'value': 0})
      $lastWrapperContent = $('<div/>', {'class': 'range-steps'})
      $.each structure.lastUpdate, (key, object) ->
        $lastWrapperContent.append $('<span/>', {'html': i18n(object.name)})
      $lastWrapperContent.appendTo $lastWrapper

      # render size
      $sizeWrapper = $('<div/>', {'class': 'filter-wrapper'})
      $sizeWrapper.append $('<div/>', {'class': 'title special', 'html': i18n('trash.trashSize')})
      $.each structure.size, (key, object) ->
        $sizeWrapper.append $('<input/>', {'type': 'checkbox', 'id': key, 'name': 'filter-size[]', 'class': 'special', 'value' : key})
        $sizeWrapper.append $('<label/>', {'for': key, 'html': i18n(object.name), 'class': 'ico ico-' + object.icon + ' ' + object.css})

      # render type
      $typeWrapper = $('<div/>', {'class': 'filter-wrapper'})
      $typeWrapper.append $('<div/>', {'class': 'title special', 'html': i18n('trash.trashType')})
      $.each structure.type, (key, object) ->
        $typeWrapper.append $('<input/>', {'type': 'checkbox', 'id': key, 'name': 'filter-type[]', 'class': 'special', 'value' : key})
        $typeWrapper.append $('<label/>', {'for': key, 'html': i18n(object.name), 'class': 'ico ico-' + object.icon})

      # render accessibility
      $accessibilityWrapper = $('<div/>', {'class': 'filter-wrapper filter-checkcs'})
      $accessibilityWrapper.append $('<div/>', {'class': 'title special', 'html': i18n('trash.accessibility')})
      $.each structure.accessibility, (key, object) ->
        $accessibilityWrapper.append $('<input/>', {'type': 'checkbox', 'id': key, 'name': 'filter-accessibility[]', 'class': 'special', 'value' : key})
        $accessibilityWrapper.append $('<label/>', {'for': key, 'html': i18n(object.name), 'class': 'ico ico-' + object.icon})
      ...
      $('#filter-button').on 'click', ->
        $(self.config.widget.id).addClass 'filter-visible'
      ...
      # update range plugin
      $('input[type="range"]').on 'change mousemove', ->
        $this = $(this)
        val = ($this.val() - $this.attr('min')) / ($this.attr('max') - $this.attr('min'))
        $this.css 'background-image', '-webkit-gradient(linear, left top, right top, ' + 'color-stop(' + val + ', #dedede), ' + 'color-stop(' + val + ', #8cc947)' + ')'
    ...
    loadFilter: () ->
      ...
      # deserialize data from URL and load them to form
      if data.length >= 5 and typeof data[4] != 'undefined' and data[4] != ''
    ...
    applyFilterTrashList: () ->
      ...
      for i of tmpData
        ...
        if typeof filter['filter-size'] != 'undefined' and $.inArray(obj.data.size, filter['filter-size']) == -1
          visibility = false
        ...
        if typeof filter['filter-accessibility'] != 'undefined' and $.fn.arrayIntersect(filter['filter-accessibility'], _accessibility).length == 0
          visibility = false

        if typeof filter['filter-type'] != 'undefined' and $.isArray(obj.data.types) and $.fn.arrayIntersect(filter['filter-type'], _.values(obj.data.types)).length == 0
          visibility = false

        # add more / less under updateNeeded
        if typeof filter['filter-status'] != 'undefined' and $.inArray('updateNeeded', filter['filter-status']) > -1
          filter['filter-status'].push('more')
          filter['filter-status'].push('less')

        if typeof filter['filter-last'] != 'undefined' and parseInt(filter['filter-last']) > 0
          # 25 - last year, 50 - last month, 75 - last week, 100 - today
          _days = 365
          if parseInt(filter['filter-last']) == 50
            _days = 31
  ...
  TrashMap.defaults = TrashMap::defaults
